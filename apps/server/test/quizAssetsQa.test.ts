...
describe("Quiz V2 assets and QA", () => {
  it("fingerprints semantic requests deterministically", () => {
    const request = {
      semantic_key: "question-01:question_illustration",
      subject: "Tiger",
      purpose: "question_illustration" as const,
  ...
  it("keeps long visual opportunities within the asset subject contract", () => {
    const longVisualOpportunity =
      "Safe illustrated dishwasher cutaway with a white rack holding plates, spray arms beneath the rack, sky-blue water jets crossing the plates, and detergent dissolving in the wash water; soft 5600K kitchen-appliance light, rounded forms, clear answer-card space, and no visible text.";
    const longQuiz = QuizV2Schema.parse({
      ...quiz,
      questions: [{ ...quiz.questions[0], format: "multiple_choice", visual_opportunity: longVisualOpportunity }],
    });
  ...
  it("creates semantic voice roles and tolerates missing decorative SFX", () => {
    ...
    expect(voice.segments.map((segment) => segment.role)).toEqual(
      expect.arrayContaining(["question", "choice", "reveal", "explanation", "outro"]),
    );
  ...
  it("changes the voice cache fingerprint when narration content changes", () => {
    ...
    expect(quizVoiceFingerprint(segment, quizVoiceTempo(segment.role), "default", config)).not.toBe(
      quizVoiceFingerprint({ ...segment, text: `${segment.text} changed` }, quizVoiceTempo(segment.role), "default", config),
    );
    expect(quizVoiceFingerprint(segment, quizVoiceTempo(segment.role), "default", config, 1.9)).not.toBe(
      quizVoiceFingerprint(segment, quizVoiceTempo(segment.role), "default", config, 2.1),
    );
  });

  it("blocks preflight when required assets are unresolved and validates render probe evidence", () => {
    const plainQuiz = QuizV2Schema.parse({
      ...quiz,
      questions: [{ ...quiz.questions[0], format: "multiple_choice", visual_opportunity: "" }],
    });
    ...
    const invalidProbe = validateRenderProbe(
      { format: { duration: "10" }, streams: [{ codec_type: "video", width: 1280, height: 720, r_frame_rate: "24/1", duration: "10" }] },
  ...
  it("strictly enforces 100% completion in isQuizAssetResolutionComplete and rejects partial resolutions", async () => {
    ...
    const assetPath1 = await repository.writeQuizImageAsset(
      channel.channel_id,
      episode.episode_id,
