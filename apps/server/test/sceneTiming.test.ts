...
describe("scene timing", () => {
  ...
  it("never combines different continuity keys", () => {
    const scenes = packBeatsIntoScenes([beat("one two", "same-place"), beat("three four", "new-place")], 8, 2.3, "episode_1");
  ...
  it("keeps editorial overlays separate from the footage prompt", () => {
    const scenes = packBeatsIntoScenes(
      [
        {
          ...beat("In 1956 the program changed", "timeline"),
          editorial_overlay: {
            kind: "timeline",
            text: "1956 — Firebird II",
            motion: "draw_on",
  ...
  it("keeps overlay coverage at or below thirty percent while preserving high-value cards", () => {
    const scenes = Array.from({ length: 10 }, (_, index) => ({
      ...
      editorial_overlay: {
        kind: index === 8 ? "stat_card" : index === 9 ? "map_callout" : index % 2 === 0 ? "caption" : "comparison",
        text: "overlay",
        motion: "fade_up",
  ...
  it("redistributes duration from a neighboring shot before leaving a sub-2.5-second cut", () => {
    const source = packBeatsIntoScenes(
      [beat("short line", "same"), beat("longer neighboring line with more words", "same-2")],
      8,
      2.3,
      "episode_1",
    );
