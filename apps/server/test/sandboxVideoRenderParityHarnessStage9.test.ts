...
import { sandboxRehearsalDocument } from "../src/quiz/render/sandbox/sandboxDocumentTemplates.js";
import { testMascotFixture, testChannelMascotConfig, renderProductionClip } from "./fixtures/sandboxParityFixtures.js";
...
describe("Stage 9: Comprehensive Regression Test Suite & Parity Harness (apps/server & apps/web)", () => {
  ...
  describe("3. Phase 2: Choices Settled Keyframe Parity (2.0s)", () => {
    it("verifies choice cards have landed and are floating at 2.0s, supporting up to 4 staggered offsets in CSS", () => {
      const { html } = renderProductionClip("full_stack_list", "multiple_choice", ["Choice A", "Choice B", "Choice C"]);
  ...
  describe("7. Multi-Layout Parity Matrix (7 Canonical Layouts)", () => {
  ...
  describe("8. Viewport Parity: 16:9 Landscape vs 9:16 Portrait", () => {
    it("calibrates dimensions, safe-zones, and element lifecycles identically across viewports", () => {
      ...
      const rehearsal16x9Html = sandboxRehearsalDocument(
        model16x9,
        parts16x9,
        stableParts16x9,
      ...
      const rehearsal9x16Html = sandboxRehearsalDocument(
        model9x16,
        parts9x16,
        stableParts9x16,
        "<div id='stage-content'></div>",
