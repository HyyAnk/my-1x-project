...
import { QuizV2Schema, type QuizPreviewLayoutId, type QuizQuestionFormat, type QuizV2 } from "@studio/shared";
...
function createTestQuiz(
  ...
  choiceTexts: string[],
  answerMode?: "multiple_choice" | "single_reveal",
): QuizV2 {
  const resolvedAnswerMode = answerMode ?? (layoutId === "mystery_reveal" ? "single_reveal" : undefined);
    ...
    questions: [
      {
        ...
        format,
        difficulty: 1,
        ...(resolvedAnswerMode ? { answer_mode: resolvedAnswerMode } : {}),
...
function renderClipHtml(layoutId: QuizPreviewLayoutId, format: QuizQuestionFormat, choices: string[]): string {
  ...
  if (layoutId === "mystery_reveal") {
    ...
    director.beats[0].answer_mode = "single_reveal";
...
describe("Stage 4: Choice Cards Staggered Entrance & Reveal Settling Synchronization", () => {
  ...
  describe("1. Staggered Entrance Animation & 2.0s Settled Keyframe Landing", () => {
    ...
    it("verifies full_stack_list has 4 distinct staggered offsets (0.00s, 0.14s, 0.28s, 0.42s) landing <= 1.81s", () => {
      const html = renderClipHtml("full_stack_list", "multiple_choice", ["Choice A", "Choice B", "Choice C"]);
  ...
  describe("3. Elimination of Premature Reveal in Scheduled Playback", () => {
    it("verifies scheduled clip playback does not leak static #22c55e border into correct card at time 0s", () => {
      ...
      expect(mysteryHtml).not.toContain(".choice-card.answer-reveal-correct { border-color: #22c55e !important; }");
      ...
      // Verify visual_choices_three_pure does not leak static green border or badge offset before reveal
      const pureHtml = renderClipHtml("visual_choices_three_pure", "odd_one_out", ["A", "B", "C"]);
      expect(pureHtml).not.toMatch(
        /\.layout-visual_choices_three_pure[^{]*\.answer-reveal-correct\s*\.option-image\s*\{[^}]*border-color:\s*#22c55e/i,
      );
      expect(pureHtml).not.toMatch(
        /\.layout-visual_choices_three_pure[^{]*\.answer-reveal-correct\s*\.choice-label\s*\{[^}]*border-color:\s*#22c55e/i,
      );
      expect(pureHtml).toContain("visual-pure-correct-badge");
      expect(pureHtml).toContain("translateX(-50%) scale(1)");
    });
  });
