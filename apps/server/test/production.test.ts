...
describe("production assessment", () => {
  it("flags an abstract clip montage as not ready", () => {
    ...
    expect(assessment.issues.map((issue) => issue.code)).toEqual(
      expect.arrayContaining(["research_sources", "script_length", "factual_density", "visual_bible"]),
    );
  });

  it("accepts a sourced, sequence-aware, continuity-locked plan", () => {
    const anchors =
      "In 1956 C01 changed the plan. In 1960 C02 proved 25 percent. In 1964 C03 recorded 30 vehicles. In 1971 C04 ended the program. In 1987 C05 changed standards. In 1997 C06 demonstrated the replacement.";
    ...
    const scenes = Array.from({ length: 10 }, (_, index) => {
      ...
      const current = scene(
        ...
        structuredPrompt(index + 1),
        `CB-${String(index + 1).padStart(2, "0")}`,
      );
      return index < 3
        ? {
            ...current,
    ...
    const assessment = assessProduction({
      ...
      visualBible:
        "# Visual Bible\n\nContinuity bundle " +
        Array.from({ length: 6 }, (_, index) => `CB-${String(index + 1).padStart(2, "0")}`).join(" "),
      script: `# Script\n\n${narration}`,
      scenes,
      fallbackWordsPerSecond: 2.3,
    });
...
describe("narration utilities", () => {
  ...
  it("keeps humor comments out of the script while restoring them for audio", () => {
    const script =
      "A serious fact lands here. <!-- AUDIO_CUE: chuckle --> The future made a very confident promise. <!-- AUDIO_CUE: laugh -->";
...
function episode(): Episode {
  return {
    ...
    slug: "episode",
    topic: { title: "Title", premise: "Premise", hook: "Hook" },
    stage: "SCENE_READY",
    script_path: "channels/channel/episodes/episode/script.md",
    research_path: "channels/channel/episodes/episode/research.md",
    treatment_path: "channels/channel/episodes/episode/treatment.md",
    visual_bible_path: "channels/channel/episodes/episode/visual_bible.md",
    scene_plan_path: "channels/channel/episodes/episode/scene_plan.md",
    dialogue_script_path: "channels/channel/episodes/episode/dialogue_script.md",
    video_prompts_path: "channels/channel/episodes/episode/video_prompts.md",
    target_duration_minutes: 3,
    target_word_count: 100,
    narration_asset_path: "channels/channel/episodes/episode/assets/narration.wav",
    narration_generated_at: "2026-08-17T00:00:00.000Z",
    narration_duration_seconds: 180,
    narration_segment_count: 6,
    measured_narration_words_per_second: 100 / 180,
...
function scene(number: number, dialogue: string, prompt: string, bundle: string): Scene {
  return {
    ...
    transition_note: "Hard cut",
    continuity_note: "Keep the locked identity",
    sequence_id: `sequence-${number}`,
    sequence_title: `Sequence ${number}`,
    shot_id: `shot-${number}`,
    asset_type: number % 3 === 0 ? "archive" : number % 3 === 1 ? "ai_reconstruction" : "diagram",
    continuity_bundle_id: bundle,
    reference_asset_ids: [`REF-${number}`],
    source_ids: [`C0${number}`],
    reconstruction: number % 3 === 1,
    sound_cue: "Low road ambience",
