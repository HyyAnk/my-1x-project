...
describe("Quiz V2 repository artifacts", () => {
  it("writes and reads versioned artifacts through fixed safe paths", async () => {
    ...
    const stored = repository.resolvePath(
      "channels",
      (await repository.getChannel(channelId)).slug,
      "episodes",
      (await repository.getEpisode(channelId, episodeId)).slug,
      "quiz",
      "quiz-v2.json",
    );
