...
describe("Quiz V2 route workflow", () => {
  it(
    "generates and persists the canonical artifact chain without a second queue",
    async () => {
  ...
  it(
    "queues the real GENERATE_VIDEO task when the V2 preflight is ready",
    async () => {
      ...
      try {
        ...
        const scenes = [
          {
            ...
            quiz: {
              ...
              answer: "Tiger",
              explanation: "Tigers have stripes.",
              image_prompt: "A friendly tiger illustration",
            },
          },
        ];
        ...
        const quiz = deriveQuizV2FromScenes({
          ...
          scenes: await app.repository.readScenes(channel.channel_id, episode.episode_id),
        });
        ...
        await app.repository.writeQuizAssetResolution(channel.channel_id, episode.episode_id, {
          schema_version: 2,
          episode_id: episode.episode_id,
          template_id: "candy_arcade",
          assets: [{ ...asset, fingerprint, path: assetPath, source: "cache" }],
        });
        ...
        await runQa({
          ...
          config: { audio_generation: (await import("../src/config.js")).DEFAULT_CONFIG.audio_generation },
          channelId: channel.channel_id,
          episodeId: episode.episode_id,
        });
        const narrationPath = await app.repository.writeQuizNarrationAudio(
          channel.channel_id,
          episode.episode_id,
          new Uint8Array([1, 2, 3]),
        );
        await app.repository.saveNarrationMetadata(
          channel.channel_id,
          episode.episode_id,
          narrationPath,
          timeline.duration_seconds,
          measuredVoice.segments.length,
          20,
        );
