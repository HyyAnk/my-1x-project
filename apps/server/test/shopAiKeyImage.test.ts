...
const originalEnv = {
  key: process.env.SHOPAIKEY_API_KEY,
  base: process.env.SHOPAIKEY_BASE_URL,
  model: process.env.SHOPAIKEY_IMAGE_MODEL,
  fallback: process.env.SHOPAIKEY_IMAGE_FALLBACK_MODEL,
  fallbacks: process.env.SHOPAIKEY_IMAGE_FALLBACK_MODELS,
  size: process.env.SHOPAIKEY_IMAGE_SIZE,
  quality: process.env.SHOPAIKEY_IMAGE_QUALITY,
};

afterEach(async () => {
  globalThis.fetch = originalFetch;
  for (const [name, value] of Object.entries({
    SHOPAIKEY_API_KEY: originalEnv.key,
    SHOPAIKEY_BASE_URL: originalEnv.base,
    SHOPAIKEY_IMAGE_MODEL: originalEnv.model,
    SHOPAIKEY_IMAGE_FALLBACK_MODEL: originalEnv.fallback,
    SHOPAIKEY_IMAGE_FALLBACK_MODELS: originalEnv.fallbacks,
    SHOPAIKEY_IMAGE_SIZE: originalEnv.size,
    SHOPAIKEY_IMAGE_QUALITY: originalEnv.quality,
  })) {
...
describe("ShopAIKey image provider", () => {
  ...
  it("posts to exactly one /v1/images/generations route and persists b64_json PNG output", async () => {
    ...
    await repository.saveTopicRun(channel.channel_id, [
      topic,
      ...Array.from({ length: 4 }, (_, index) => ({ ...topic, topic_id: `shopaikey_topic_${index + 2}` })),
    ]);
    ...
    const asset = await new ShopAiKeyImageProvider(repository, {
      ...
      bundleNumber: 1,
      variant: 0,
    }).generateReference("Anchor prompt");
  ...
  it("retries transient provider failures before persisting the image", async () => {
    ...
    await repository.saveTopicRun(channel.channel_id, [
      topic,
      ...Array.from({ length: 4 }, (_, index) => ({ ...topic, topic_id: `retry_topic_${index + 2}` })),
    ]);
    const episode = await repository.confirmTopic(channel.channel_id, topic.topic_id);
    process.env.SHOPAIKEY_API_KEY = "test-key";
    const png = "iVBORw0KGgo=";
  ...
  it("falls back to the configured model after primary model timeouts", async () => {
    ...
    await repository.saveTopicRun(channel.channel_id, [
      topic,
      ...Array.from({ length: 4 }, (_, index) => ({ ...topic, topic_id: `fallback_topic_${index + 2}` })),
    ]);
    const episode = await repository.confirmTopic(channel.channel_id, topic.topic_id);
    process.env.SHOPAIKEY_API_KEY = "test-key";
