...
export async function tryDeleteThread(this: TaskManagerRuntime, threadId: string, engine?: "codex" | "antigravity"): Promise<boolean> {
  ...
  if (targetEngine === "antigravity") {
    ...
    try {
      ...
      this.logger.debug(`Antigravity session cleanup skipped: ${error instanceof Error ? error.message : "unknown error"}`, {
        step: "antigravity_thread_cleanup",
      });
      return false;
    }
  }
  try {
    return await this.codex.deleteThread(threadId);
  } catch (error) {
