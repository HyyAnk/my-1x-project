  ...
  try {
    ...
    if (layoutReady) {
      ...
      for (let attempt = 1; attempt <= maxCheckAttempts; attempt++) {
        const checkInvocation = getHyperframesInvocation("check", renderRoot, "--json", "--samples", "5", "--timeout", String(checkTimeoutMs));
        try {
          ({ stdout: checkOutput } = await execFileAsync(
            checkInvocation.command,
            checkInvocation.args,
    ...
    if (reusableRender) {
      ...
      await execFileAsync(
        renderInvocation.command,
        renderInvocation.args,
        {
