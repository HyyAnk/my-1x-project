...
export function analyzeWavAudio(buffer: Uint8Array, expectedRanges: Array<{ start: number; end: number }> = []): VoiceAudioDiagnostics {
  ...
  const windowFrames = Math.max(1, Math.floor(sampleRate * 0.05));
  const lowThreshold = 0.012;
  ...
  return {
    ...
    low_audio_ratio: Number((lowDuration / Math.max(0.001, duration)).toFixed(4)),
    speech_occupancy: Number((1 - lowDuration / Math.max(0.001, duration)).toFixed(4)),
...
export function audioDiagnosticsForTimeline(buffer: Uint8Array, timeline: QuizTimeline): VoiceAudioDiagnostics {
  ...
  diagnostics.narration_segment_energy = narration.map((event) => ({
    segment_id: event.segment_id!,
    energy: measureRangeEnergy(buffer, event.at_seconds, event.at_seconds + event.duration_seconds),
  }));
  const reveal = diagnostics.narration_segment_energy.filter((item) => /:reveal$/.test(item.segment_id)).map((item) => item.energy);
  const explanation = diagnostics.narration_segment_energy
    .filter((item) => /:(explanation|fact)$/.test(item.segment_id))
    .map((item) => item.energy);
  if (reveal.length && explanation.length)
    diagnostics.reveal_energy_to_explanation_energy_ratio = Number((average(reveal) / Math.max(0.0001, average(explanation))).toFixed(3));
  return diagnostics;
}
...
function measureRangeEnergy(buffer: Uint8Array, startSeconds: number, endSeconds: number): number {
  ...
  for (let frame = first; frame < last; frame += 1)
    for (let channel = 0; channel < channels; channel += 1) {
      sum += Math.abs(view.getInt16(dataOffset + frame * frameSize + channel * 2, true) / 32768);
      count += 1;
    }
...
function mergeWindows(
  ...
  for (const window of windows) {
    ...
    if (last && window.start <= last.end_seconds + 0.051) last.end_seconds = window.end;
  ...
  for (const run of runs) {
    ...
    run.expected = expectedRanges.some((range) => {
      ...
      return overlap >= run.duration_seconds * 0.5;
    });
...
function readWavFormat(buffer: Uint8Array): {
  sampleRate: number;
  channels: number;
  ...
  if (
    ...
    new TextDecoder().decode(buffer.slice(8, 12)) !== "WAVE"
  )
    throw new Error("Audio diagnostics received an invalid WAV file");
  const view = new DataView(buffer.buffer, buffer.byteOffset, buffer.byteLength);
  let offset = 12;
  let sampleRate = 0;
  let channels = 0;
  ...
  if (!sampleRate || !channels || !dataOffset || !dataSize)
