...
export function compileVideoDescriptionPrompt(input: CompileVideoDescriptionPromptInput): string {
  ...
  return [
    ...
    `5. SCORING CTA: Use the calculated ranges (${tier1Range}, ${tier2Range}, ${tier3Range}) and assign engaging rank titles in ${language} (e.g. "Novice", "Scholar", "Master"). Add an inviting comment prompt.`,
    `6. PLAYLIST THEME: State 1 clean playlist category name (plain text only, NEVER invent or write URLs).`,
    `7. HASHTAGS: Provide 3-5 relevant hashtags: 1 niche brand/topic tag, 2 content keyword tags, and 1 general quiz tag (e.g. #quiz, #trivia, #education).`,
    `8. DIVERSITY & ANTI-DUPLICATION: Use fresh, engaging phrasing instead of generic template cliches.`,
    `9. AUDIENCE COMPLIANCE: ${isPreschool ? "Preschool/young child audience allowed." : "Do NOT use sensitive keywords like 'for kids', 'toddler', 'preschool' unless strictly applicable."}`,
