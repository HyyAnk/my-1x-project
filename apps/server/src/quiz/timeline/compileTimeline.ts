...
export function compileQuizTimeline(input: TimelineCompileInput): QuizTimeline {
  ...
  add({
    ...
    choice_id: null,
    segment_id: null,
    payload: { theme: input.director.archetype_family },
  });
  ...
  for (const [questionIndex, question] of input.quiz.questions.entries()) {
    ...
    const questionNarrationDuration = questionSegment
      ? scheduleNarration(questionSegment.segment_id, questionNarrationAt, questionSegment.text, question.id)
      : 0;
    ...
    const thinkingNarrationDuration = thinkingSegment
      ? scheduleNarration(thinkingSegment.segment_id, thinkingStart, thinkingSegment.text, question.id)
      : 0;
    ...
    const thinkingSeconds = round(
      Math.min(
        policy.maximum_thinking_seconds,
        Math.max(policy.minimum_thinking_seconds, beat.thinking_seconds, minRequiredThinkingSeconds),
      ),
    );
    add({
    ...
    const countdownNarrationDuration = countdownSegment
      ? scheduleNarration(countdownSegment.segment_id, countdownStart, countdownSegment.text, question.id)
      : 0;
    ...
    if (revealSegment) {
      ...
      revealNarrationEnd =
        revealNarrationAt + scheduleNarration(revealSegment.segment_id, revealNarrationAt, revealSegment.text, question.id);
    }
    ...
    cursor = round(postReveal + Math.max(0.05, policy.transition_seconds - policy.transition_overlap_seconds));
  }
  ...
  if (outro) {
    ...
    add({
      ...
      duration_seconds: round(cursor - outroStart),
      question_id: null,
      choice_id: null,
      segment_id: null,
  ...
  const sorted = events
    ...
    .map(({ event }) => ({ ...event, at_seconds: round(event.at_seconds), duration_seconds: round(event.duration_seconds) }));
  return QuizTimelineSchema.parse({
    schema_version: 2,
    episode_id: input.quiz.episode_id,
    duration_seconds: Math.max(0.1, round(cursor)),
    events: sorted,
  });
}
