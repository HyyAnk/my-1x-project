...
export function assessQuiz(input: QuizAssessmentInput): QuizAssessment {
  ...
  if (input.quiz.questions.length >= 5 && mostCommonPosition / input.quiz.questions.length > 0.6) {
    ...
    add({
      ...
      question_ids: input.quiz.questions
  ...
  if (input.resolvedAssets) {
    for (const asset of input.resolvedAssets as Array<{
      asset_id: string;
      path: string;
      source: string;
      degraded?: boolean;
  ...
  if (input.timeline) {
    ...
    const starts = input.quiz.questions
      .map(
        (question) =>
          input.timeline!.events.find((event) => event.type === "question.enter" && event.question_id === question.id)?.at_seconds,
      )
    ...
    if (rushed.length)
      add({
        ...
        severity: "warning",
        message: `${rushed.length} question cycle${rushed.length === 1 ? " is" : "s are"} outside the ${minimumCycle}–${maximumCycle}s target for ${input.quiz.age_band}.`,
  ...
  const rating =
    !hasBlocker && score >= 85 && candyArcadeVisual.total >= 85 ? "production_ready" : score >= 70 ? "needs_review" : "not_ready";
