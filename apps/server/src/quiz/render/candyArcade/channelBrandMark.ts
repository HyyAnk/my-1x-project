  ...
  return `function fitChannelBrandMarks() {
    try {
      ...
      elements.forEach(function(el) {
        ...
        if (isPortrait) {
          const sub = parent.querySelector('.brand-mark-sub');
          const subWidth = sub ? sub.offsetWidth : 0;
          const maxWidth = 640 - subWidth - 16;
          let size = 42;
          const minSize = 20;
          ...
          const maxWidth = (parent.clientWidth ? parent.clientWidth - 24 : 0) || 296;
          let size = 84;
          const minSize = 36;
          el.style.fontSize = size + 'px';
          while (el.scrollWidth > maxWidth && size > minSize) {
