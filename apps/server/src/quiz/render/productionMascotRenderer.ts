 ...
 * - If the mascot lacks a dedicated wave action, intro clip uses the active style's celebrate[0] (or thinking[0] or master concept).
 * - If the mascot lacks a dedicated outro action, outro clip uses the active style's celebrate[1] (or celebrate[0] or master concept).
 * - If the mascot lacks thinking or celebrate in a question clip, falls back to active style's first variant.
 */
export function adaptMascotForPhase(
  ...
  if (phase === "intro") {
    if (!hasDedicatedAction(mascot, "wave")) {
      ...
      const fallbackUrl = fallbackVariant?.image_url ?? mascot.master_image_url?.trim() ?? "";
      if (fallbackUrl) {
        newActions.wave = {
          action: "wave",
    ...
    if (!hasDedicatedAction(mascot, "thinking") && thinkingVariants[0]) {
      const v = thinkingVariants[0];
      newActions.thinking = {
        action: "thinking",
        sprite_url: v.image_url,
        preview_url: v.image_url,
        frames_count: 1,
        fps: 8,
        loop: true,
        frame_width: 512,
        ...
        offset_x: 0,
        offset_y: 0,
        motion_preset: v.motion_preset ?? "sway",
        motion_speed: v.motion_speed ?? 1.0,
        motion_intensity: v.motion_intensity ?? "normal",
      };
    }
    if (!hasDedicatedAction(mascot, "celebrate") && celebrateVariants[0]) {
      const v = celebrateVariants[0];
      newActions.celebrate = {
        action: "celebrate",
        sprite_url: v.image_url,
        preview_url: v.image_url,
        ...
        offset_x: 0,
        offset_y: 0,
        motion_preset: v.motion_preset ?? "jump",
        motion_speed: v.motion_speed ?? 1.0,
        motion_intensity: v.motion_intensity ?? "normal",
      };
    }
