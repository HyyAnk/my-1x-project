...
function buildQuizOutputContract(input: OutputContractInput): string | null {
...
function buildStandardOutputContract(input: OutputContractInput): string {
  const {
    taskType,
    isQuiz,
    episode,
...
export function buildOutputContract(input: OutputContractInput): string {
  if (input.isQuiz) {
    const quizContract = buildQuizOutputContract(input);
    if (quizContract) return quizContract;
  }
  return buildStandardOutputContract(input);
}
