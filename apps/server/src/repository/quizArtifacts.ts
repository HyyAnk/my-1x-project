...
import {
  DirectorPlanSchema, EpisodeSchema, QuizAssessmentSchema, QuizAssetPlanSchema, QuizAssetResolutionSchema, QuizTimelineSchema, QuizV2Schema, QuestionHistoryCheckResultSchema, QuestionHistoryEntrySchema, BgmHistoryEntrySchema, VoicePlanSchema, nowIso,
  type DirectorPlan, type QuizAssessment, type QuizAssetPlan, type QuizAssetResolution, type QuizQuestion, type QuizTimeline, type QuizV2, type QuestionHistoryCheckResult, type QuestionHistoryEntry, type BgmHistoryEntry, type VoicePlan,
} from "@studio/shared";
...
export async function writeDirectorPlan(this: RepositoryRuntime,channelId: string, episodeId: string, plan: DirectorPlan): Promise<string> {
...
export async function readQuizAssetResolution(this: RepositoryRuntime,channelId: string, episodeId: string): Promise<QuizAssetResolution | null> {
  return this.readQuizArtifact(channelId, episodeId, "asset-resolution.json", QuizAssetResolutionSchema);
}

export async function writeQuizAssetResolution(this: RepositoryRuntime,channelId: string, episodeId: string, resolution: QuizAssetResolution): Promise<string> {
  return this.writeQuizArtifact(channelId, episodeId, "asset-resolution.json", QuizAssetResolutionSchema.parse(resolution));
}

export async function writeQuizImageAsset(this: RepositoryRuntime,channelId: string, episodeId: string, assetId: string, fingerprint: string, content: Uint8Array, meta?: BundleImageMeta): Promise<string> {
...
export async function resolveQuizAssetPath(this: RepositoryRuntime,channelId: string, episodeId: string, assetPath: string): Promise<string> {
  const episode = await this.getEpisode(channelId, episodeId);
  const channel = await this.getChannel(channelId);
...
export async function writeQuizTimeline(this: RepositoryRuntime,channelId: string, episodeId: string, timeline: QuizTimeline): Promise<string> {
...
export async function writeQuizAssessment(this: RepositoryRuntime,channelId: string, episodeId: string, assessment: QuizAssessment): Promise<string> {
...
export async function readHistoryCheck(this: RepositoryRuntime,channelId: string, episodeId: string): Promise<QuestionHistoryCheckResult | null> {
  return this.readQuizArtifact(channelId, episodeId, "history-check.json", QuestionHistoryCheckResultSchema);
}

export async function writeHistoryCheck(this: RepositoryRuntime,channelId: string, episodeId: string, result: QuestionHistoryCheckResult): Promise<string> {
...
export async function appendQuestionHistory(this: RepositoryRuntime,channelId: string, episodeId: string, questions: QuizQuestion[], ttlDays = 30): Promise<void> {
  const channel = await this.getChannel(channelId);
...
export async function appendBgmHistory(this: RepositoryRuntime,channelId: string, episodeId: string, trackId: string, filename: string, ttlDays = 30): Promise<void> {
  const channel = await this.getChannel(channelId);
  const episode = await this.getEpisode(channelId, episodeId).catch(() => null);
  const episodeTitle = episode?.topic?.title || episodeId;
  const historyPath = this.resolvePath("channels", channel.slug, "bgm_history.json");
...
export async function invalidateQuizArtifacts(this: RepositoryRuntime,channelId: string, episodeId: string, stages: string[]): Promise<string[]> {
