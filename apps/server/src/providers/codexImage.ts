...
export class CodexImageProvider implements ImageProvider {
  ...
  async generateReference(_prompt: string): Promise<{ asset_path: string }> {
    ...
    if (reportedPath) {
      ...
      try {
        return {
          asset_path: await this.repository.writeBundleImageFromFile(
            this.target.channelId,
            this.target.episodeId,
            this.target.bundleNumber,
            resolvedPath,
    ...
    throw new Error(
      `Image generation returned no PNG at ${destination.path}. The connected Codex account/model may be text-only; configure an image-capable provider (for example, an OpenAI-compatible image service) or disable continuity anchor images.`,
    );
  }
}
