...
test("Quiz Engine V2 rail is status-only and Build video is the single production action", async ({ page }) => {
  const channel = {
    channel_id: "ch_quiz_v2",
    slug: "quiz-v2",
    display_name: "Quiz V2 channel",
    description: "Quiz",
    target_audience: "Children",
    language: "English",
    market: "Global",
    channel_dna_path: "channels/quiz-v2/channel_dna.md",
  ...
  const episode = {
    ...
    slug: "quiz-story",
    topic: { title: "Quiz Story", premise: "A deterministic quiz episode.", hook: "Can you spot the answer?" },
    stage: "VIDEO_READY",
    script_path: "channels/quiz-v2/episodes/quiz-story/script.md",
    research_path: "channels/quiz-v2/episodes/quiz-story/research.md",
    treatment_path: "channels/quiz-v2/episodes/quiz-story/treatment.md",
    visual_bible_path: "channels/quiz-v2/episodes/quiz-story/visual_bible.md",
    scene_plan_path: "channels/quiz-v2/episodes/quiz-story/scene_plan.md",
    dialogue_script_path: "channels/quiz-v2/episodes/quiz-story/dialogue_script.md",
    video_prompts_path: "channels/quiz-v2/episodes/quiz-story/video_prompts.md",
    ...
    video_asset_path: "channels/quiz-v2/episodes/quiz-story/assets/quiz-video.mp4",
    video_generated_at: channel.updated_at,
    video_duration_seconds: 42,
    render_manifest_path: "channels/quiz-v2/episodes/quiz-story/assets/render-manifest.json",
    created_at: channel.created_at,
    updated_at: channel.updated_at,
  };
  const v2 = {
    quiz: { schema_version: 2, episode_id: episode.episode_id, age_band: "7-9", language: "English", questions: [] },
    director_plan: null,
    asset_plan: null,
    voice_plan: null,
    timeline: null,
  ...
  const taskList: Array<Record<string, unknown>> = [];
  ...
  await page.route("**/api/tasks", (route) =>
    route.fulfill({ contentType: "application/json", body: JSON.stringify({ tasks: taskList, codex_status: "connected" }) }),
  );
  ...
  for (const filename of ["research.md", "treatment.md", "script.md", "visual_bible.md"])
    await page.route("**/api/channels/" + channel.channel_id + "/episodes/" + episode.episode_id + "/file/" + filename, (route) =>
      route.fulfill({
        contentType: "application/json",
        body: JSON.stringify({ content: "# " + filename, path: filename, modified_at: episode.updated_at }),
      }),
    );
  ...
  await page.route("**/api/channels/" + channel.channel_id + "/episodes/" + episode.episode_id + "/quiz-v2", (route) =>
    route.fulfill({ contentType: "application/json", body: JSON.stringify(v2) }),
  );
  let folderOpened = false;
  await page.route("**/api/channels/" + channel.channel_id + "/episodes/" + episode.episode_id + "/video/open-folder", async (route) => {
    folderOpened = route.request().method() === "POST";
    await route.fulfill({
      contentType: "application/json",
      body: JSON.stringify({ opened: true, folder_path: "channels/quiz-v2/episodes/quiz-story/assets" }),
    });
  });
