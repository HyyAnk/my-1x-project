...
import { ArrowsClockwise, DeviceMobile, Eye, EyeSlash, Monitor, Sparkle, VideoCamera } from "@phosphor-icons/react";
  ...
  if (!question) {
    return (
      <div className="qb-preview-empty">
        <div className="qb-preview-empty-icon">👈</div>
    ...
    <div className="qb-preview-card">
      <div className="qb-preview-header">
        ...
        <div className="qb-preview-controls">
          ...
          >
            {isShorts ? <Monitor size={15} /> : <DeviceMobile size={15} />}
            <span>{aspect}</span>
          </button>
          ...
          >
            {showAnswer ? <EyeSlash size={15} /> : <Eye size={15} />}
      ...
      {/* 1-Click Quick Build Video Action */}
      {onQuickBuildVideo && (
        <button
          type="button"
