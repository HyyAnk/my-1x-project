...
export function getUnitTone(state: ReelUnitStatus, isPending: boolean): "ready" | "pending" | "failed" | "stale" | "missing" | "neutral" {
  if (isPending) return "pending";
  if (state === "cancelled") return "neutral";
  return state;
}

export function resolveStageStatuses(units: ShortReelRecord["units"], activeTask?: Task | null, isGenerating = false) {
