...
const BUILT_IN_PRESETS_DEF: Array<
...
export function VisualSandboxTab({
  ...
  const [mascotId, setMascotId] = useState<string>("fallback"); // "none", "fallback", or mascot ID
  ...
  const [mascotOffsetX, setMascotOffsetX] = useState<number>(0);
  const [mascotOffsetY, setMascotOffsetY] = useState<number>(0);

  // Timeline & Playback State
  const [phase, setPhase] = useState<"question" | "choices" | "thinking" | "reveal" | "explain">("thinking");
  const [timelineSeconds, setTimelineSeconds] = useState<number>(3.5);
  const [isPlaying, setIsPlaying] = useState<boolean>(false);
  const [useScrubber, setUseScrubber] = useState<boolean>(false);
  ...
  const [correctChoiceIndex, setCorrectChoiceIndex] = useState<number>(sampleQuestions[0].correct);
  const [factCardTitle, setFactCardTitle] = useState<string>(sampleQuestions[0].fact_title || (language === "vi" ? "BẠN CÓ BIẾT?" : "DID YOU KNOW?"));
  const [factCardText, setFactCardText] = useState<string>(sampleQuestions[0].fact_text || (language === "vi" ? "Vành đai của Sao Thổ chủ yếu cấu tạo từ hàng tỷ hạt băng, bụi và đá vụn!" : "Saturn's rings are made mostly of billions of ice particles and rocky debris!"));
  const [questionNumber, setQuestionNumber] = useState<number>(1);
  const [totalQuestions, setTotalQuestions] = useState<number>(10);

  // Viewport & Overlays
  const [showSafeArea, setShowSafeArea] = useState<boolean>(false);
  const [showShortsGuide, setShowShortsGuide] = useState<boolean>(false);
  ...
  const [syncMascotToChannel, setSyncMascotToChannel] = useState<boolean>(true);
  const [savingChannel, setSavingChannel] = useState(false);

  const containerRef = useRef<HTMLDivElement>(null);
  const [scaleFactor, setScaleFactor] = useState<number>(0.5);

  // Fetch real mascots on mount
  useEffect(() => {
    api
      .mascots()
      .then((res) => {
    ...
    async (manualNotice = false) => {
      ...
      try {
        const input: SandboxPreviewInput = {
          ...
          phase,
          timeline_time_seconds: useScrubber ? timelineSeconds : undefined,
          question_text: questionText,
          choices,
          correct_choice_index: correctChoiceIndex,
          question_number: questionNumber,
          total_questions: totalQuestions,
          countdown_progress: Math.max(0, Math.min(1, (timelineSeconds - 2.5) / 5.0)),
          fact_card_title: factCardTitle,
          fact_card_text: factCardText,
          mascot_id: mascotId === "none" ? null : mascotId === "fallback" ? "fallback" : mascotId,
          mascot_enabled: mascotEnabled && mascotId !== "none",
          mascot_action: mascotAction,
          mascot_position: mascotPosition,
          mascot_scale: mascotScale,
          mascot_offset_x: mascotOffsetX,
          mascot_offset_y: mascotOffsetY,
        };
        ...
        if (manualNotice && onNotice) {
          onNotice({
            tone: "good",
            message: t("visualSandbox.noticeRerendered", { time: timeStr }),
          });
        }
      } catch (err) {
        if (onNotice) {
  ...
  const handleLoadPreset = (preset: VisualPresetItem) => {
    ...
    setLayoutId(preset.layout_id || "media_left_choices_right");
    setThinkingBarStyle(preset.thinking_bar_style);
    setQuestionBoxStyle(preset.question_box_style);
    setAnswerCardStyle(preset.answer_card_style || "glossy_arcade");
    setCounterStyle(preset.counter_style);
    if (preset.mascot_id !== undefined) setMascotId(preset.mascot_id || "none");
    if (preset.mascot_position) setMascotPosition(preset.mascot_position);
    if (preset.mascot_scale !== undefined) setMascotScale(preset.mascot_scale);
    if (preset.mascot_offset_x !== undefined) setMascotOffsetX(preset.mascot_offset_x);
    if (preset.mascot_offset_y !== undefined) setMascotOffsetY(preset.mascot_offset_y);
  ...
  const handleSaveCustomPreset = () => {
    ...
    if (onNotice) {
      onNotice({ tone: "good", message: t("visualSandbox.noticeSavedPreset", { name: newPreset.name }) });
    }
  };
  ...
  const handleApplyToChannel = async () => {
    ...
    try {
      ...
      await api.updateChannel(selectedChannelId, {
        ...
        default_answer_card_style: answerCardStyle,
        default_counter_style: counterStyle,
        default_palette_id: paletteId,
      });

      // 2. Sync Mascot to Channel if selected
      if (syncMascotToChannel && mascotId && mascotId !== "fallback") {
        if (mascotId === "none") {
          ...
          await api.assignMascotToChannel(selectedChannelId, {
            ...
            config: {
              ...
              scale: mascotScale,
              offset_x: mascotOffsetX,
              offset_y: mascotOffsetY,
      ...
      if (onNotice) {
        onNotice({
          tone: "good",
          message: t("visualSandbox.noticeAppliedToChannel", { name: targetChannel.display_name }),
        });
      }
    } catch (err) {
      if (onNotice) {
        onNotice({
          tone: "bad",
          message: err instanceof Error ? err.message : t("visualSandbox.noticeSaveError"),
        });
      }
    } finally {
  ...
  const handlePhaseChange = (newPhase: "question" | "choices" | "thinking" | "reveal" | "explain") => {
    setUseScrubber(false);
    setIsPlaying(false);
    setPhase(newPhase);
    if (newPhase === "question") setTimelineSeconds(0.5);
    else if (newPhase === "choices") setTimelineSeconds(1.8);
    else if (newPhase === "thinking") setTimelineSeconds(4.5);
    else if (newPhase === "reveal") setTimelineSeconds(8.0);
    else if (newPhase === "explain") setTimelineSeconds(9.2);
  };
          ...
          >
            ...
            {matchedPreset ? (
              ...
              >
                ...
                >
                  {matchedPreset.isBuiltIn
                    ? t("visualSandbox.presetBuiltInBadge")
                    : t("visualSandbox.presetCustomBadge")}
                </span>
              ...
              <div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "6px", marginBottom: "4px" }}>
                  {ALL_THINKING_BAR_STYLES.filter((s) => s !== "auto").map((style) => {
                    ...
                    const label = THINKING_BAR_STYLE_LABELS[style as Exclude<QuizThinkingBarStyle, "auto">] || style;
                    return (
                      <button
                        key={style}
                        type="button"
              ...
              <div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "6px", marginBottom: "4px" }}>
                  {ALL_QUESTION_BOX_STYLES.filter((s) => s !== "auto").map((style) => {
                    ...
                    const label = QUESTION_BOX_STYLE_LABELS[style as Exclude<QuizQuestionBoxStyle, "auto">] || style;
                    return (
                      <button
                        key={style}
                        type="button"
              ...
              <div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "6px", marginBottom: "4px" }}>
                  {ALL_ANSWER_CARD_STYLES.filter((s) => s !== "auto").map((style) => {
                    ...
                    const label = ANSWER_CARD_STYLE_LABELS[style as Exclude<QuizAnswerCardStyle, "auto">] || style;
                    return (
                      <button
                        key={style}
                        type="button"
              ...
              <div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "6px", marginBottom: "4px" }}>
                  {ALL_QUESTION_COUNTER_STYLES.filter((s) => s !== "auto").map((style) => {
                    ...
                    const label = QUESTION_COUNTER_STYLE_LABELS[style as Exclude<QuizQuestionCounterStyle, "auto">] || style;
                    return (
                      <button
                        key={style}
                        type="button"
                ...
                <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
                  <div style={{ flex: 1 }}>
                    <span style={{ fontSize: "10.5px", color: "var(--muted)" }}>
                      {t("visualSandbox.currentQuestionNumber")}
                    </span>
                    <input
                      type="number"
                      min={1}
                  ...
                  <div style={{ flex: 1 }}>
                    <span style={{ fontSize: "10.5px", color: "var(--muted)" }}>
                      {t("visualSandbox.totalQuestionsCount")}
                    </span>
                    <input
                      type="number"
                      min={1}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                  <div>
                    <span style={{ display: "block", fontSize: "10.5px", color: "var(--muted)", marginBottom: "4px" }}>
                      {t("visualSandbox.factCardTitleLabel")}:
                    </span>
                    <input
                      type="text"
                      className="text-input compact"
                      value={factCardTitle}
                      onChange={(e) => setFactCardTitle(e.target.value)}
                      placeholder={t("visualSandbox.factCardTitlePlaceholder")}
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
              ...
              <button
                type="button"
                className="quiet-button compact"
                style={{ fontSize: "11px", padding: "4px 8px", color: "#38BDF8", display: "inline-flex", alignItems: "center", gap: "4px" }}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "12px" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                <button
                  ...
                  style={{ fontSize: "11px", padding: "4px 12px", display: "inline-flex", alignItems: "center", gap: "5px", borderRadius: "8px" }}
                  onClick={() => {
                    setUseScrubber(true);
                    setIsPlaying((p) => !p);
                  }}
        ...
        >
          <div className="panel" style={{ width: "420px", padding: "24px", borderRadius: "16px" }} onClick={(e) => e.stopPropagation()}>
            ...
            <p style={{ fontSize: "13px", color: "var(--muted)", marginBottom: "16px" }}>
              {t("visualSandbox.modalSavePresetDesc")}
            </p>
      ...
      {channelSyncOpen && (
        ...
        >
          <div className="panel" style={{ width: "480px", padding: "24px", borderRadius: "16px" }} onClick={(e) => e.stopPropagation()}>
            ...
            <p style={{ fontSize: "13px", color: "var(--muted)", marginBottom: "16px" }}>
              {t("visualSandbox.modalApplyChannelDesc")}
            </p>
            ...
            {mascotId !== "fallback" && (
              <label style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "12px", marginBottom: "14px", cursor: "pointer" }}>
                <input
                  type="checkbox"
                  checked={syncMascotToChannel}
                  onChange={(e) => setSyncMascotToChannel(e.target.checked)}
                />
                <span>
            ...
            >
              ...
              <div>
                <strong>• {t("visualSandbox.summaryPalette")}</strong>{" "}
                {PALETTES.find((p) => p.id === paletteId)?.label || paletteId}
              </div>
              <div>
