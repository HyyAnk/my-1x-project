...
export function useTasks(onTerminal?: (task: Task) => void) {
  ...
  const upsertTask = useCallback((task: Task) => {
    ...
    setTasks((current) =>
      [task, ...current.filter((item) => item.task_id !== task.task_id)].sort((a, b) => b.created_at.localeCompare(a.created_at)),
    );
  }, []);
  const refresh = useCallback(async () => {
    ...
    if (sequence === refreshSequence.current && revision === dataRevision.current) {
      setTasks(response.tasks);
      setCodexStatus(response.codex_status);
    }
    return response;
  }, []);
  useEffect(() => {
