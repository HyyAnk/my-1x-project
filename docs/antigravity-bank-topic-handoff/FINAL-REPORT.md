...
- **Live Status:** Migration applied to `.quiz-studio/knowledge_base/` on 2026-09-08.
- **Backup Location:** `.quiz-studio/knowledge_base/.migration_backup_1725806892/`.
- **Entities & Combos:** 2,500 questions across 250 batch files.
- **Index Integrity:** Rebuilt `index.json` reflects exact archetype distribution with matching hash signatures.
- **Byte Proof:** Zero source question text, options, or explanations modified; zero translations written to Question Bank storage.
- **Rollback Procedure:** If rollback is ever required:
  ```bash
  node scripts/migration/rollback-bank-metadata.mjs --backup .quiz-studio/knowledge_base/.migration_backup_1725806892
  ```
