# Execute Short-Reel Phase 03: Mixed Topics And Bank Selection
...
Execute ONLY Phase 03 of the Short-Reel implementation kit. This prompt authorizes bounded work in that phase, not all later phases. You may be Codex, Antigravity or another coding agent; repository artifacts are the source of truth, not chat history or tool-specific memory.
...
5. `docs/short-reel-implementation/phases/03-topics-selection.md`.
...
7. Predecessor phase `phases/02-contracts-storage.md`, its implementation/review evidence and registry release record.

## Entry Gate

Phase 02 must be implemented, verified and released. If its review is outstanding, first review its actual code/diff and evidence using `prompts/reviewer.md`; record acceptance under a documentation claim and release it before Phase 03 product edits. If review finds a blocking defect or the predecessor is incomplete, stop and report the exact repair needed. Do not repair an earlier phase silently under this phase's claim.

If Phase 03 is already partially implemented, reconcile current code/evidence and resume unfinished slices only. If it is already accepted, verify the record and report that fact; do not reimplement it or automatically start the next phase.
...
Deliver 3:2 topics, one keyword-directed candidate per pillar and exactly one approved source question. Confirm by explicit content kind and provide a working minimal draft destination, not an Episode substitute.

## Required Finish

Write `docs/short-reel-implementation/verification/evidence/phase-03-implementation.md` using the evidence template and `docs/agent-coordination/handoffs/short-reel-phase-03.md` using the repository handoff template. Include changed files, tests/exit codes, runtime workflow, mocked versus real boundaries, blockers and next-agent input. Update progress before final verification. Do not copy secrets into evidence.
