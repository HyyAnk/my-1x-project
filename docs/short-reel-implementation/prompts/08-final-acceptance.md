# Execute Short-Reel Phase 08: Integration And Final Acceptance
...
Execute ONLY Phase 08 of the Short-Reel implementation kit. This prompt authorizes bounded work in that phase, not all later phases. You may be Codex, Antigravity or another coding agent; repository artifacts are the source of truth, not chat history or tool-specific memory.
...
5. `docs/short-reel-implementation/phases/08-final-acceptance.md`.
...
7. Predecessor phase `phases/07-portrait-retirement.md`, its implementation/review evidence and registry release record.

## Entry Gate

Phase 07 must be implemented, verified and released. If its review is outstanding, first review its actual code/diff and evidence using `prompts/reviewer.md`; record acceptance under a documentation claim and release it before Phase 08 product edits. If review finds a blocking defect or the predecessor is incomplete, stop and report the exact repair needed. Do not repair an earlier phase silently under this phase's claim.

If Phase 08 is already partially implemented, reconcile current code/evidence and resume unfinished slices only. If it is already accepted, verify the record and report that fact; do not reimplement it or automatically start the next phase.
...
Run full regression and both primary workflows, then collect actual manual Flow evidence from the user for both archetypes. Keep final acceptance pending until explicitly granted; never fabricate footage checks or user approval.

## Required Finish

Write `docs/short-reel-implementation/verification/evidence/phase-08-implementation.md` using the evidence template and `docs/agent-coordination/handoffs/short-reel-phase-08.md` using the repository handoff template. Include changed files, tests/exit codes, runtime workflow, mocked versus real boundaries, blockers and next-agent input. Update progress before final verification. Do not copy secrets into evidence.
...
Report result, evidence/handoff links, checks actually run, remaining issues and release status. Distinguish technical completion, manual Flow review and final user acceptance.
