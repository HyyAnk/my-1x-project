# Execute Short-Reel Phase 07: Controlled Portrait Retirement
...
Execute ONLY Phase 07 of the Short-Reel implementation kit. This prompt authorizes bounded work in that phase, not all later phases. You may be Codex, Antigravity or another coding agent; repository artifacts are the source of truth, not chat history or tool-specific memory.
...
5. `docs/short-reel-implementation/phases/07-portrait-retirement.md`.
...
7. Predecessor phase `phases/06-studio-integration.md`, its implementation/review evidence and registry release record.

## Entry Gate

Phase 06 must be implemented, verified and released. If its review is outstanding, first review its actual code/diff and evidence using `prompts/reviewer.md`; record acceptance under a documentation claim and release it before Phase 07 product edits. If review finds a blocking defect or the predecessor is incomplete, stop and report the exact repair needed. Do not repair an earlier phase silently under this phase's claim.

If Phase 07 is already partially implemented, reconcile current code/evidence and resume unfinished slices only. If it is already accepted, verify the record and report that fact; do not reimplement it or automatically start the next phase.
...
Re-review fresh exact removal/data manifests before deletion. Purge legacy portrait quiz/stage/Sandbox paths but retain generic portrait media, Short-Reel assets and protected data. No broad root cleanup or compatibility layer.

## Required Finish

Write `docs/short-reel-implementation/verification/evidence/phase-07-implementation.md` using the evidence template and `docs/agent-coordination/handoffs/short-reel-phase-07.md` using the repository handoff template. Include changed files, tests/exit codes, runtime workflow, mocked versus real boundaries, blockers and next-agent input. Update progress before final verification. Do not copy secrets into evidence.
