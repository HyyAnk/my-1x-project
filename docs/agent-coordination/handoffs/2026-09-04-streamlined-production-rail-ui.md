...
- **Problem**: In Phase 4, `PipelineRail.tsx` was streamlined, but `QuizEpisodeView.tsx` actually mounted `QuizV2Panel.tsx`, which was still rendering 12 stages (2 rows × 6 columns), including 5 legacy pre-production steps (`Research`, `Treatment`, `Script`, `Visual bible`, `Scenes`).
- **Solution**:
...
- [`apps/web/src/components/QuizV2Panel.test.tsx`](file:///d:/1a/Cursor/Project/My/1x/Project/apps/web/src/components/QuizV2Panel.test.tsx):
  - 3 unit tests verifying 4-stage streamlined rendering, omission of legacy retired stages, 12-stage fallback, and running task note.
