    ...
    try:
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            if src_ratio > target_ratio:
                ...
                # Source is taller than 16:9 -> crop top/bottom (bias slightly toward top for faces: 25% top, 75% bottom)
...
if __name__ == '__main__':
    imgs = get_album_images("/terminator-desktop-wallpapers", 8)
    print(f"Found {len(imgs)} candidate images for Terminator")
    for i, u in enumerate(imgs):
        out = f"tmp/inspect_candidates/terminator_{i+1}.png"
        ok, orig_size = download_and_crop(u, out)
        if ok:
            print(f"Saved candidate {i+1}: {out} (from {u}, orig: {orig_size})")
