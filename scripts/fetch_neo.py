    ...
    try:
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            if src_ratio > target_ratio:
                ...
                top = max(0, int((src_h - new_h) * 0.15))
...
if __name__ == '__main__':
    imgs = get_album_images("/the-matrix-neo-wallpapers", 6)
    imgs += get_album_images("/the-matrix-neo-desktop-wallpapers", 6)
    print(f"Found {len(imgs)} candidate images for Neo")
    idx = 1
    for u in imgs:
        out = f"tmp/inspect_candidates/neo_{idx}.png"
